...
namespace YG
{
    ...
    public partial class InfoYG : ScriptableObject
    {
        ...
        [Tooltip("Отвечает за очерёдность загрузки игры инициалпзацию.\n\nLoad Game Run = false:\nсначала будет происходить полная инициализация SDK Яндекса, затем только будет загружаться игра.\n\nLoad Game Run = false:\nИгра будет загружаться вместе с инициализацией SDK Яндекса. В таком случае, игра может загрузиться раньше инициализации.")]
